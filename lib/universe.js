(function() {
  ...
  module.exports = Universe = (function() {
    ...
    Universe.prototype.startGame = function() {
      this.stopped = false;
      this.players[this.self.id] = this.self;
      this.enableControls();
      return this.gameTick();
    };
    ...
    Universe.prototype.connect = function() {
      ...
      this.socket.on('connect', __bind(function() {
        this.socket.send({
          source: this.self.id,
          action: "initSelf",
          data: this.self.serialized(true)
        });
    ...
    Universe.prototype.drawHealthMeter = function() {
      ...
      this.context.fillStyle = "rgba(255,255,255,0.2)";
      this.context.fillRect(x, y, 100, 5);
      percent = this.self.damage / constants.deadlyDamage;
      this.context.fillStyle = percent < .8 ? "green" : "red";
      return this.context.fillRect(x, y, 100 - 100 * percent, 5);
    };
