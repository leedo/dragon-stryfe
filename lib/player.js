(function() {
  ...
  module.exports = Player = (function() {
    ...
    Player.prototype.handleDeath = function() {
      if (this.damage > constants.deadlyDamage || this.dead) {
        this.dead++;
        this.position.angle = Math.PI * this.dead / 10.0;
        if (this.dead === 1) {
          console.log("" + this.name + " died at tick " + this.tick_count);
          this.damage = "dead";
          return this.trail = [];
        } else if (this.dead >= constants.deathAnimationTime) {
          this.damage = 0;
          this.dead = 0;
          this.position.x = Math.random() * constants.universeWidth;
          this.position.y = Math.random() * constants.universeHeight;
          return this.flash = 1;
        }
      }
    };
    ...
    Player.prototype.draw = function(context) {
      ...
      if (this.flash > 4) {
        ...
        this.context.arc(this.self.position.x, this.self.position.y, 30, 0, 2 * Math.PI, false);
        this.context.fill();
