(function() {
  var OAuth, addGame, app, broadcast, constants, express, fs, games, io, next_id, object_id, openGame, package, redis, redis_client, send, send_powerup, socket, stitch, stream, util;
  ...
  addGame = function(logname) {
    ...
    if (constants.fullLogs) {
      newgame.log = fs.createWriteStream(logname, {
        encoding: 'utf8'
      });
    }
    games.push(newgame);
