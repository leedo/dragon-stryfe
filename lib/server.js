(function() {
  var OAuth, addGame, app, broadcast, constants, express, fs, games, io, next_id, object_id, openGame, package, redis, redis_client, send, send_powerup, socket, stitch, util;
  ...
  openGame = function() {
    var game, gamecount, p, playercount, players, _i, _j, _len, _len2, _ref;
    players = 0;
    gamecount = 0;
      ...
      switch (msg.action) {
        case "initSelf":
          ...
          if (game.authed[self.id]) {
            return redis_client.get("ds-" + self.player.name, function(err, res) {
              if (!err && res > 0) {
